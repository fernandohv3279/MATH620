...
For n=0 (constant polynomials).

We want to find $c_0$ and $C_0$ so that
\[
	c_0||u||_1\leq ||u||_2 \leq C_0||u||_1.
\]
Because $||u||_2=||u||_1+||u'||_1$ and $u'=0$ we can take
$c_0=C_0=1$.

Now for $n$

We want to find $c_n$ and $C_n$ so that
\[
	c_n||u||_1\leq ||u||_2 \leq C_n||u||_1.
\]
Because $||u||_2=||u||_1+||u'||_1$ we can take $c_n=1$.

As for $C_n$ we see that
\begin{align*}
\Big|\Big|\sum_0^n a_ix^i\Big|\Big|_2 &\leq ||a_nx^n||_2 + \Big|\Big|\sum_0^{n-1}a_ix^i\Big|\Big|_2\\
&= |a_n| + n|a_n| + \Big|\Big|\sum_0^{n-1}a_ix^i\Big|\Big|_2\\
&\leq |a_n| + n|a_n| + C_{n-1}\Big|\Big|\sum_0^{n-1}a_ix^i\Big|\Big|_1\\
&\leq \max\{C_{n-1},n+1\} \left(|a_n| + \Big|\Big|\sum_0^{n-1}a_ix^i\Big|\Big|_1\right)
\end{align*}
\section*{Problem 4}
\end{document}
